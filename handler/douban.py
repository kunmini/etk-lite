...
class DoubanApi:
    ...
    # 以下为豆瓣官方 App 的公共密钥（非用户私有，全网开源项目通用）
        ...
        if candidates:
            ...
            if year:
                ...
                if year_exact_candidates:
                    # ★ 改进：按片名相似度排序（original_title/中文名 与搜索名比对），选最像的
                    #   避免 "America" 匹配到《中国合伙人》这类错误
                    import difflib
                    def _score(cand):
                        n = str(name).strip().lower()
                        best = 0.0
                        for field in ("original_title", "title"):
                            t = str(cand.get(field) or "").strip().lower()
                            if not t:
                                continue
                            if t == n:
                                return 1.0
                            # 英文名比对（忽略空格/符号差异）
                            best = max(best, difflib.SequenceMatcher(None, n, t).ratio())
                        return best
                    ranked = sorted(year_exact_candidates, key=_score, reverse=True)
